...
/**
 * @brief This is the complex control that allows the user to edit and run the StickWorld. Probably a
...
class StickWorldNode : public HybridNode<B> {
   ...
   public:
    ...
    virtual void finalize() {
        ...
        // This pointer allows the StickWorld to see its StickWorldNode and automatically update it when it
        // changes state.
...
class StickWorld {
   public:
    StickWorld(int gridWidth, int gridHeight, int gridPixelWidth, int gridPixelHeight,
    ...
    /**
     * @brief Creates a new smaller StickWorldNode to serve as a reaction pattern to run in the main
     * grid.
     *
     */
    void makeNewReactionRule() {
        StickWorld *reactionRule = new StickWorld(this->ruleGridWidth_, this->ruleGridHeight_, 150, 150,
                                          "rule-" + clto_str(++this->ruleCount_), true);
        this->reactionRules_.push_back(reactionRule);

        CLNodeFactory<StickWorldNode, StickWorld<V>, int> beakerBuilder("div", "rr");

        StickWorldNode<StickWorld<V>> *bn = beakerBuilder.withChildrenOf(beakerNode_->reactionRulesDiv_)
                                        .withTag("div")
    ...
    static void makeNewReactionRule_st(StickWorld *b) { b->makeNewReactionRule(); }

   protected:
    string name_;
    bool isReactionRule_ = false;  //!< Set to true if this StickWorld is being used as a reaction rule
                                   //!< for an enclosing StickWorld.
    ...
    StickWorldNode<StickWorld<V>> *beakerNode_;  //!< Pointer back to containing BN so that BN->refresh()
                                         //!< can be called when this updates.

    vector<StickWorld *> reactionRules_;

    StickWorld *successor_;           //!< The pattern we replace this one with.
...
EMSCRIPTEN_BINDINGS(PixelReactor) {
    ...
    class_<StickWorldNode<StickWorld<unsigned char>>>("BeakerNode_h")
        .function("doNothing", &StickWorldNode<StickWorld<unsigned char>>::doNothing, allow_raw_pointers());
...
struct Matchsticks : public PageContent {
    ...
    ClarityNode *content(ClarityNode *innerContent = nullptr) {
        ...
        StickWorld<unsigned char> *b = new StickWorld<unsigned char>(60, 40, 600, 400, "StickWorld");
